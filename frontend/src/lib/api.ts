...
export interface ActivityStatus {
  ...
  resume_last_analyzed_at: string | null;
  resume_skills_detected: number;
  github_connected: boolean;
  github_last_analyzed_at: string | null;
...
export const authApi = {
  ...
  registerStudent: (payload: object) =>
    api.post("/auth/register/student", payload),
  registerCollege: (payload: object) =>
...
export const studentApi = {
  ...
  joinCollege: (collegeId: number) =>
    api.post(`/students/me/join-college?college_id=${collegeId}`),
  myProfile: () => api.get<StudentProfile>("/students/me/profile"),
  updateProfile: (
    payload: Partial<{
      branch: string;
      year_of_study: number;
  ...
  ) => api.put<StudentProfile>("/students/me/profile", payload),
  activityStatus: () => api.get<ActivityStatus>("/students/me/activity-status"),
};

// ---------- Skill Gap ----------
export const gapApi = {
  roles: () => api.get("/gap/roles"),
  report: (roleTitle: string) =>
    api.get<GapReport>(
...
export const opportunityApi = {
  ...
  apply: (opportunityId: number) =>
    api.post<ApplicationOut>("/opportunities/apply", {
      opportunity_id: opportunityId,
    }),
  myApplications: () =>
    api.get<ApplicationOut[]>("/opportunities/applications/mine"),
  applicationsFor: (opportunityId: number) =>
    api.get<ApplicationOut[]>(`/opportunities/${opportunityId}/applications`),
  updateStatus: (applicationId: number, status: string) =>
    api.patch(`/opportunities/applications/${applicationId}/status`, {
      status,
    }),
  submitFeedback: (payload: object) =>
    api.post("/opportunities/feedback", payload),
};
...
export const aiApi = {
  status: () =>
    api.get<{ ai_configured: boolean; provider: string }>("/ai/status"),
  analyzeResume: (file: File) => {
    const form = new FormData();
    form.append("file", file);
    return api.post("/ai/resume/analyze", form, {
  ...
  analyzeEvidence: (evidenceId: number) =>
    api.post(`/ai/evidence/${evidenceId}/analyze`),
  adaptiveStart: (skillName: string) =>
    api.post("/ai/assessment/adaptive/start", { skill_name: skillName }),
  adaptiveNext: (sessionId: number, answer: string[]) =>
    api.post("/ai/assessment/adaptive/next", { session_id: sessionId, answer }),
  roadmapGenerate: (targetCareer: string, weeklyHours = 6) =>
    api.post("/ai/roadmap/generate", {
